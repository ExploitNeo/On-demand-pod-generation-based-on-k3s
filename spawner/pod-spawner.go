...
func handleConnection(userConn net.Conn, clientset *kubernetes.Clientset) {
	...
	pod, err := createDynamicPod(clientset, podName)
	if err != nil {
...
func createDynamicPod(clientset *kubernetes.Clientset, name string) (*v1.Pod, error) {
    challengeImage := os.Getenv("CHALLENGE_IMAGE")
    if challengeImage == "" {
        challengeImage = "localhost/ota_agent:latest"
    ...
    pod := &v1.Pod{
        ...
        Spec: v1.PodSpec{
            ...
            Containers: []v1.Container{
                {
                    Name:            "ota-agent", // RFC1123 OK
                    Image:           challengeImage,
                    ImagePullPolicy: v1.PullNever,
                    SecurityContext: &v1.SecurityContext{
                        Privileged:             &privileged, // nested podman 위해 필요
